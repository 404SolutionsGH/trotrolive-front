...
export default function Map({ startStation, destinationStation }: MapProps) {
    ...
    if (startStation && destinationStation) {
      ...
      if (
        ...
        L.marker([startLat, startLng], { icon: startIcon })
          ...
          .bindPopup(
            `<strong>Start: ${startStation.name}</strong><br>${startStation.station_address}`,
          );
        L.marker([destLat, destLng], { icon: destIcon })
          .addTo(routeLayerRef.current)
          .bindPopup(
            `<strong>Destination: ${destinationStation.name}</strong><br>${destinationStation.station_address}`,
          );
        L.polyline(
          [
            [startLat, startLng],
  ...
  useEffect(() => {
    ...
    loadLeaflet()
      .then((L) => {
        ...
        // Draw route if stations are present
        updateRoute();
      })
      .catch((err) => {
  ...
  useEffect(() => {
    updateRoute();
  }, [startStation, destinationStation, mapRef.current, routeLayerRef.current]);
